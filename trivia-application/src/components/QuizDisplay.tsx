...
const QuizDisplay: React.FC<QuizDisplayProps> = (props: QuizDisplayProps) => {
  ...
  const [quizScore, setQuizScore] = useState<Array<boolean>>(
    new Array(props.results.length),
  );
        ...
        {showAnswers ? (
          <section className="flex justify-center m-5">
            <article>
              <ol>
                {props.results.map((question, index) => (
                  <li key={question.question} className="text-l w-200 py-2">
                    ...
                    <label
                      className={
                        (quizScore[index] ? "text-green-500" : "text-red-600") +
                        " font-bold"
                      }
                    >
              ...
              <p className="py-5">
                {props.userName} scored:{" "}
                {quizScore.filter((score) => score == true).length} points! out
                of a {quizScore.length} possible points.
              </p>
              <Button onClick={() => props.resetQuiz()} className="text-black">
                Back To Selection
              </Button>
              {highScores.length > 0 ? (
                <>
                  <p className="py-5 font-bold text-xl">
                    High Scores for:{" "}
                    {formatCategory(props.category, props.categoryData)}
                  </p>
                  {highScores.map((score: Score, index: number) => (
                    <div key={index}>
                      {index < 5 && (
                        <p>
                          {score.userName} scored: {score.score} points!
                        </p>
                      )}
                    </div>
                  ))}
          ...
          <form
            onSubmit={quizForm.handleSubmit(() => setshowAnswers(true))}
            className="flex justify-center m-40"
          >
            <Label htmlFor="name" className="text-2xl">
              Thats all the questions! Are you ready to see how you did?
            </Label>
            <Button type="submit" className="m-10 text-black">
              Submit Answers
            </Button>
          </form>
        )}
  ...
  return (
    <>
      <QuizQuestion
        question={props.results[quizIndex].question}
        correct_answer={props.results[quizIndex].correct_answer}
        incorrect_answers={props.results[quizIndex].incorrect_answers}
