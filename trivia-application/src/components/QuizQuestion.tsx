...
const QuizQuestion: React.FC<QuizQuestionProps> = (
  ...
  const jumbleAnswers = () => {
    const correctIndex = Math.floor(
      Math.random() * props.incorrect_answers.length + 1,
    );
    setCorrectAnswerIndex(correctIndex);
    const answers: Array<QuizAnswers> = new Array<QuizAnswers>(
      props.incorrect_answers.length + 1,
    );
          ...
          <article className="flex justify-center">
            {answers.map((answer, index) => (
              <Button
                key={answer.answer}
                onClick={() =>
                  props.answerQuestion(index == correctAnswerIndex)
                }
                type="submit"
                className="text-gray-800 m-10"
              >
                {parse(answer.answer)}
              </Button>
            ))}
