    ...
    <>
      <Card className="bg-cover aspect-[15/16] py-0 flex shadow-md justify-between bg-slate-700 m-30">
        <CardTitle className="text-white font-bold text-lg">
          <p className="bg-linear-to-t from-0% to-gray-950 py-5 rounded-t-xl px-5">
            {props.id || "No Id."}
          </p>
        </CardTitle>
        ...
        <CardDescription className="text-lg text-slate-300 p-5">
          {submittedData.map((submitted: number) => (
            <a
              ...
              href={`https://hacker-news.firebaseio.com/v0/item/${submitted}.json?print=pretty`}
              target="_blank"
            >
              <p>{`https://hacker-news.firebaseio.com/v0/item/${submitted}.json?print=pretty`}</p>
            </a>
          ))}
        </CardDescription>
